...
import { Trash2, ShoppingBag, ArrowLeft, ArrowRight, CheckCircle2, Loader2, Shield, Truck, CreditCard, Banknote, Lock } from "lucide-react";
import { useState } from "react";
import api from "@/lib/api";
import { getProductImage } from "@/utils/productImages";
import { MastercardIcon, VisaIcon, AmexIcon } from "@/components/ui/payment-icons";
...
export default function CheckoutPage() {
    ...
    const validateCard = () => {
        ...
        if (!cardName.trim()) { setCardError("Enter the cardholder name"); return false; }
        if (cardExpiry.length < 5) { setCardError("Enter a valid expiry date"); return false; }
        if (cardCvv.length < 3) { setCardError("Enter a valid CVV"); return false; }
        setCardError("");
        return true;
    };
                            ...
                            <div className="space-y-4 text-sm font-medium">
                                ...
                                <div className="border-t border-[#E5E7EB] pt-6 space-y-4">
                                    ...
                                    <div className="grid grid-cols-1 gap-3">
                                        ...
                                        {/* Card Details Form */}
                                        {paymentMethod === "CARD" && (
                                            <div className="bg-gray-50 border-2 border-[#DC2626]/20 rounded-xl p-4 space-y-3 animate-in fade-in duration-300">
                                                <div className="flex items-center justify-between mb-1">
                                                    <p className="text-[10px] font-black uppercase tracking-widest text-gray-500">Card Details</p>
                                                ...
                                                <div className="relative">
                                                    <input
                                                        ...
                                                        className="w-full bg-white border-2 border-gray-200 rounded-lg px-4 py-2.5 text-sm font-bold tracking-widest focus:outline-none focus:border-[#DC2626] transition-colors"
                                                        maxLength={19}
                                                    />
                                                    {getCardType(cardNumber) === "visa" && <span className="absolute right-3 top-2.5 text-[10px] font-black text-blue-600 uppercase">VISA</span>}
                                                    {getCardType(cardNumber) === "mastercard" && <span className="absolute right-3 top-2.5 text-[10px] font-black text-red-500 uppercase">MC</span>}
                                                </div>
                                                <input
                                                    type="text"
                                                    ...
                                                    className="w-full bg-white border-2 border-gray-200 rounded-lg px-4 py-2.5 text-sm font-bold uppercase tracking-wider focus:outline-none focus:border-[#DC2626] transition-colors"
                                                />
                                                <div className="grid grid-cols-2 gap-3">
                                                    ...
                                                    <div className="relative">
                                                        <input
                                                            ...
                                                            maxLength={4}
                                                        />
                                                        <Lock className="absolute right-3 top-3 w-3.5 h-3.5 text-gray-400" />
                                                    </div>
                                                </div>
                                                {cardError && <p className="text-xs text-red-500 font-bold">{cardError}</p>}
